...
public class CourseService
{
    ...
    public async Task<CourseResult> GetCourseAsync(string category = "", string searchQuery = "", int pageNumber = 1, int pageSize = 10)
    {
        var response = await _httpClient.GetAsync($"{_configuration["ApiUris:Courses"]}?key={_configuration["ApiKey"]}&category={Uri.UnescapeDataString(category)}&searchQuery={Uri.UnescapeDataString(searchQuery)}&pageNumber={pageNumber}&pageSize={pageSize}");
